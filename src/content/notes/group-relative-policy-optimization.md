...
deterministic. If we start with a prompt $x$ and our LLM predicts tokens $t_1$ and $t_2$ given
this prompt as input, then our updated state simply becomes $s_2 = \{x, t_1, t_2\}$. In other
words, our state is just the running completion being generated by the LLM for a given prompt
$x$.
