...
Under Pipeline RL, one rollout looks like this instead:

```text
Prompt
  π_t      → Token 1, Token 2
  (weight update)
  π_{t+1}  → Token 3, Token 4
  (weight update)
  π_{t+2}  → Token 5, Token 6
```

What belongs in the denominator? It can't be $\pi_t$, because tokens 3 and 4 weren't generated by $\pi_t$. It can't be $\pi_{t+1}$ either, because tokens 1 and 2 weren't. There is no single old policy for the whole rollout.

Pipeline RL's answer is to track the behavior policy at a finer granularity: every token (or segment of tokens) records which policy version generated it, and the ratio is computed per token,

$$
r_i = \frac{\pi_\theta(a_i \mid s_i)}{\pi_{\text{behavior}, i}(a_i \mid s_i)}
$$

where the denominator now changes from token to token. Each token carries its own behavior policy, so training uses the correct denominator for each one. One rollout = one behavior policy becomes one token = one behavior policy.
